...
Configuration file names are based on the template's package name.
Scoped package names have their leading `@` removed and `/` replaced with `-`: for example, a `@scope/create-example` template would read from a `scope-create-example.config.*` file.
