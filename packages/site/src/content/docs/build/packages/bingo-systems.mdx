...
`glob` resolves with the paths of files matching one or more [glob patterns](https://github.com/SuperchupuDev/tinyglobby#readme), relative to the current working directory.

#### `createReadingFileSystem`

Creates a new `ReadingFileSystem` instance.
Internally, it calls to [`node:fs`](https://nodejs.org/api/fs.html) to read directories and files, and to [`tinyglobby`](https://github.com/SuperchupuDev/tinyglobby) to match globs.
...
Internally, it calls to [`node:fs`](https://nodejs.org/api/fs.html) to read and write directories and files, and to [`tinyglobby`](https://github.com/SuperchupuDev/tinyglobby) to match globs.
