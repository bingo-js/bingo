...
Passing `colors: true` asks the command to print colors, which commands otherwise skip because they aren't attached to a TTY.
It's ignored if the terminal doesn't support colors.
