...
:::note
Windows file modes don't include executable bits, so Node.js can't tell whether a file is executable.
On Windows, files are read in without `executable` metadata: `["..."]` instead of `["...", { executable: false }]`.
:::
