...
The `fs` object contains the properties of a [`ReadingFileSystem`](/build/packages/bingo-systems#readingfilesystem):

- `glob`: Given one or more glob patterns, returns a Promise for the matching paths
- `readDirectory`: Given a directory path, returns a Promise for the names of its children
...
Inputs with a generic call signature, such as [`input-from-octokit`](https://www.npmjs.com/package/input-from-octokit), have their result type inferred from the provided `args`.
Those `args` are inferred as `const`, so such Inputs should accept readonly arrays.
