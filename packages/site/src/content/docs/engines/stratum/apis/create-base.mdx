...
- `produce` _(optional)_: a [Block production](/engines/stratum/concepts/blocks#production) method
...
Block Definitions may include a `produce()` function for their core logic.
Blocks that only add creations in [setup](#createblock-setup) or [transition](#createblock-transition) mode can omit it.
